...
public class BordersDAO {
	...
	public List<Border> isCollegate(int anno) {
		...
		try {
			...
			while (rs.next()) {
				//System.out.format("%d %s %s\n", rs.getInt("ccode"), rs.getString("StateAbb"), rs.getString("StateNme"));
				Border b = new Border(rs.getInt("s1"), rs.getInt("s2"));
				result.add(b);
			}
	...
	public Map<Integer, Country> countryCollegate(int anno){
		
		String sql="Select * "
				+ "from country c "
				+ "where c.CCode in( "
				+ "Select state1no as s1 "
				+ "From contiguity c "
				+ "where c.conttype = 1 and c.year<=? "
				+ ")";
